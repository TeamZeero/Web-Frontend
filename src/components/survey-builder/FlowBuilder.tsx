...
export function FlowBuilder({
  ...
  const [toolMode, setToolMode] = useState<"none" | "cut" | "reattach">("none")
  ...
  const buildLinearEdges = useCallback((allNodes: Node[]) => {
    ...
    if (questionNodes.length > 0) {
      edges.push({ id: `edge-start-${questionNodes[0].id}`, source: "start", target: questionNodes[0].id, type: "smoothstep" })
      for (let i = 0; i < questionNodes.length - 1; i++) {
        edges.push({ id: `edge-${questionNodes[i].id}-${questionNodes[i + 1].id}`, source: questionNodes[i].id, target: questionNodes[i + 1].id, type: "smoothstep" })
      }
      edges.push({ id: `edge-${questionNodes[questionNodes.length - 1].id}-end`, source: questionNodes[questionNodes.length - 1].id, target: "end", type: "smoothstep" })
    } else {
  ...
  useEffect(() => {
    ...
    setNodes((prevNodes) => {
      ...
      const startEnd = prevNodes.filter((n) => n.type === "startNode" || n.type === "endNode")

      // map existing question nodes by id for position/data preservation
      const prevQuestionNodes = prevNodes.filter((n) => n.type === "questionNode")
      const prevById = new Map(prevQuestionNodes.map((n) => [n.id, n]))

      // build question nodes based on incoming list; but if enforceLinear is false, preserve existing order
      let questionNodes: Node[] = []

      if (enforceLinear) {
        // follow incoming order, prefer existing node position, then saved question.position, then default
        questionNodes = (questions as any[]).map((q: any, i: number) => {
          const existing = prevById.get(q.id)
          return {
            id: q.id,
            type: "questionNode",
            position: existing?.position ?? q.position ?? { x: 250, y: 120 + i * 120 },
            data: {
              questionType: q.type,
              title: q.title,
        ...
        // preserve existing order of prevQuestionNodes, include any new incoming nodes at the end
        questionNodes = prevQuestionNodes

        // add any new questions that don't exist yet
        for (const q of (questions as any[])) {
          if (!prevById.has(q.id)) {
            questionNodes.push({
              id: q.id,
              type: "questionNode",
              position: q.position ?? { x: 250, y: 150 + questionNodes.length * 120 },
              data: {
                questionType: q.type,
                title: q.title,
                description: q.description,
        ...
        questionNodes = questionNodes.filter((n) => incomingSet.has(n.id))
      }

      const newNodes = [...startEnd, ...questionNodes]
      ...
      if (enforceLinear) {
        ...
        setEdges((prevEdges) => {
          ...
          return prevEdges.filter((e) => (nodeIds.has(String(e.source)) || e.source === "start") && (nodeIds.has(String(e.target)) || e.target === "end"))
        })
      }
  ...
  const onDropCanvas = useCallback(
    (e: React.DragEvent) => {
      e.preventDefault()
      const type = e.dataTransfer.getData("application/question-type") || e.dataTransfer.getData("text/plain")
      if (!type) return
      // compute accurate drop position relative to the ReactFlow viewport using the instance
      const bounds = reactFlowWrapper.current?.getBoundingClientRect()
      const clientX = e.clientX
      const clientY = e.clientY
      let position = { x: clientX - (bounds?.left || 0) - 100, y: clientY - (bounds?.top || 0) - 20 }

      if (reactFlowInstance.current && typeof reactFlowInstance.current.project === "function" && bounds) {
        const projected = reactFlowInstance.current.project({ x: clientX - bounds.left, y: clientY - bounds.top })
      ...
      if (enforceLinear) {
        ...
        setEdges((eds) => {
          const filtered = eds.filter((e) => !(e.target === "end" && String(e.source).startsWith("question-")))
        ...
        if (onQuestionsChange) {
          const questions = nodes
            ...
            .map((node) => ({
              ...
              scaleMinLabel: node.data.scaleMinLabel,
              scaleMaxLabel: node.data.scaleMaxLabel,
  ...
  const addQuestionNode = useCallback(
    (type: string) => {
      ...
      const yPosition = questionNodes.length > 0 ? Math.max(...questionNodes.map((node) => node.position.y)) + 120 : 150
      ...
      const newNode: Node = {
        ...
        data: {
          ...
          required: false,
          options:
            type === "multipleChoice" || type === "checkboxes" || type === "dropdown" ? ["Option 1", "Option 2", "Option 3"] : [],
          scaleMin: type === "linearScale" ? 1 : undefined,
          scaleMax: type === "linearScale" ? 5 : undefined,
          scaleMinLabel: type === "linearScale" ? "Strongly Disagree" : undefined,
          scaleMaxLabel: type === "linearScale" ? "Strongly Agree" : undefined,
        },
      }
      setNodes((nds) => nds.concat(newNode))
      ...
      if (enforceLinear) {
        ...
        // Ensure only the newest question connects directly to end
        setEdges((eds) => {
          const filtered = eds.filter((e) => !(e.target === "end" && String(e.source).startsWith("question-")))
          return [...filtered, endEdge]
        })
      } else {
        // when not enforcing linear path, do not auto-create edges; preserve existing edges
        // nothing to do here
      }
    },
  ...
  const updateNodeData = useCallback(
    (nodeId: string, newData: any) => {
      setNodes((nds) => {
        const updated = nds.map((node) => (node.id === nodeId ? { ...node, data: { ...node.data, ...newData } } : node))
  ...
  const cutNode = useCallback((nodeId: string) => {
    ...
    setEdges((eds) => {
      const toRemove = eds.filter((e) => e.source === nodeId || e.target === nodeId)
      if (toRemove.length > 0) {
        savedEdgesRef.current[nodeId] = toRemove
        console.debug("FlowBuilder cutNode - saved edges for", nodeId, toRemove)
      } else {
  ...
  const reattachNode = useCallback((nodeId: string) => {
    ...
    if (saved && saved.length > 0) {
      setEdges((eds) => {
        ...
        console.debug("FlowBuilder reattachNode - restoring saved edges for", nodeId, toAdd)
        return [...eds, ...toAdd]
      })
    ...
    if (enforceLinear) {
      setEdges((eds) => {
        const questionNodes = nodes.filter((n) => n.type === "questionNode" && n.id !== nodeId)
        const newEdges: Edge[] = []

        if (questionNodes.length > 0) {
          const last = questionNodes[questionNodes.length - 1]
          newEdges.push({ id: `edge-${last.id}-${nodeId}`, source: last.id, target: nodeId, type: "smoothstep" })
        } else {
          newEdges.push({ id: `edge-start-${nodeId}`, source: "start", target: nodeId, type: "smoothstep" })
        }

        newEdges.push({ id: `edge-${nodeId}-end`, source: nodeId, target: "end", type: "smoothstep" })

        console.debug("FlowBuilder reattachNode - auto connecting", nodeId, newEdges)
        return [...eds, ...newEdges]
      })
    } else {
      // when not enforcing linear path, do not auto-connect as a fallback
      console.debug("FlowBuilder reattachNode - enforceLinear is false, skipping auto-connect for", nodeId)
    }
  }, [nodes, setEdges])
  ...
  const onNodeMouseDown = useCallback((event: React.MouseEvent, node: Node) => {
    ...
    nodeMouseDownPosRef.current = { x: (event as any).clientX, y: (event as any).clientY }
    pendingSourceRef.current = node.id
    console.debug("FlowBuilder onNodeMouseDown", node.id)
  }, [])

  const onNodeMouseUp = useCallback((event: React.MouseEvent, node: Node) => {
    console.debug("FlowBuilder onNodeMouseUp", node.id, "pending=", pendingSourceRef.current)
  ...
  const onSelectionChange = useCallback((selection: { nodes: Node[]; edges: Edge[] } | null) => {
  ...
  const onWrapperPointerDebug = useCallback((e: React.MouseEvent) => {
    ...
    console.debug("FlowBuilder wrapper event", e.type, "target=", (e.target as HTMLElement)?.className)
  }, [])
    ...
    <div className="h-full flex">
      <div className="flex-1 relative">
        <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
          <Card className="px-3 py-2 shadow-lg">
            <div className="flex items-center gap-4 text-sm">
              ...
              <Button size="sm" variant="outline" onClick={generateShareUrl} className="h-7 bg-transparent">
                📋 Copy Link
              </Button>
              <Button
                size="sm"
                onClick={handlePublish}
                disabled={isPublishing || nodes.filter((n) => n.type === "questionNode").length === 0}
                className="h-7 bg-blue-600 hover:bg-blue-700 text-white"
              >
                {isPublishing ? "Publishing..." : "🚀 Publish"}
              </Button>
              {shareUrl && <span className="text-green-600 text-xs">Link copied!</span>}
            </div>
          </Card>
        </div>
        ...
        <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
          <Card className="px-3 py-2 shadow-lg">
            <div className="flex items-center gap-2">
              <Button size="sm" variant={toolMode === "cut" ? "default" : "outline"} onClick={() => { setToolMode(toolMode === "cut" ? "none" : "cut"); pendingSourceRef.current = null }}>
                ✂️ Cut
              </Button>
              <Button size="sm" variant={toolMode === "reattach" ? "default" : "outline"} onClick={() => { setToolMode(toolMode === "reattach" ? "none" : "reattach"); pendingSourceRef.current = null }}>
                🔗 Reattach
              </Button>
              <Button size="sm" variant={enforceLinear ? "default" : "outline"} onClick={() => setEnforceLinear((v) => !v)}>
  ...
  <div ref={(el) => { reactFlowWrapper.current = el }} className="h-full w-full" onClick={onWrapperPointerDebug} onMouseDown={onWrapperPointerDebug}>
          <ReactFlow
            onInit={(instance) => {
          ...
          onMouseDown={(e: unknown) => console.debug("FlowBuilder onMouseDown", e)}
          onNodeDragStop={(event, node) => {
            ...
            setNodes((nds) => {
              ...
              const questionNodes = updated.filter((n) => n.type === "questionNode").slice()
              questionNodes.sort((a, b) => (a.position?.y || 0) - (b.position?.y || 0))
        ...
        >
          ...
          <MiniMap className="bg-white shadow-lg rounded-lg" nodeColor="#6366f1" maskColor="rgba(99, 102, 241, 0.1)" />
          <Background variant={BackgroundVariant.Dots} gap={20} size={1} color="#e0e7ff" />
        </ReactFlow>
        </div>
      </div>
